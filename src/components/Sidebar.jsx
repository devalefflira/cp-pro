...
export default function Sidebar() {
  ...
  const [loadingAdminAction, setLoadingAdmin] = useState(false);
  ...
  // FUNÇÃO DE ADMINISTRADOR: DESLOGAR TODOS OS OUTROS USUÁRIOS
  const handleDeslogarTodos = async () => {
    if (!confirm("Tem certeza que deseja deslogar todos os usuários ativos do sistema?")) return;

    setLoadingAdmin(true);
    const { error } = await supabase.rpc('invalidar_sessoes_usuarios');

    if (error) {
      alert("Erro ao invalidar sessões: " + error.message);
    } else {
      alert("Todas as sessões de usuários comuns foram deslogadas com sucesso!");
    }
  ...
  return (
    <aside className={`bg-[#003366] text-white min-h-screen p-4 flex flex-col justify-between transition-all duration-300 relative ${collapsed ? 'w-20' : 'w-64'}`}>
      ...
      <div className="pt-4 border-t border-blue-900 space-y-2">
        {/* BOTÃO ADMINISTRATIVO EXCLUSIVO DO ADMIN */}
        {userProfile?.email === 'admin@cppro.com' && (
          <button
          ...
          >
            ...
            {!collapsed && <span>{loadingAdminAction ? "Encerrando..." : "Deslogar Todos Usuários"}</span>}
          </button>
        )}
