...
import { PlusCircle, Save, X, Plus, Search, ChevronDown } from 'lucide-react';
...
export default function LancarTab({ irParaListagem }) {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      setFornecedores(resF.data || []); setCentrosCusto(resCC.data || []); setGrupos(resG.data || []); setContas(resC.data || []);
    }
    carregar();
  }, []);
  ...
  const handleSalvarDespesa = async (e) => {
    ...
    else { alert("Sucesso!"); setForm(formInicial); irParaListagem(); }
  };
  ...
  const handleSalvarFornecedor = async () => {
    ...
    if (error) alert("Erro: " + error.message);
      ...
      setModalForn(false); setNovoForn({nome:'', cpf_cnpj:''});
    }
  };
    ...
    <div className="bg-white p-8 rounded-xl shadow-md border border-gray-200">
      <form onSubmit={handleSalvarDespesa} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <select className="w-full p-3 border rounded bg-gray-50 focus:ring-2 disabled:opacity-50" value={form.grupo_id} onChange={(e) => setForm({ ...form, grupo_id: e.target.value, conta_id: '' })} disabled={!form.centro_custo_id} required>
            <option value="">Selecione...</option>
        ...
        <div className="md:col-span-2">
          ...
          <select className="w-full p-3 border rounded bg-gray-50 focus:ring-2 disabled:opacity-50" value={form.conta_id} onChange={(e) => setForm({ ...form, conta_id: e.target.value })} disabled={!form.grupo_id} required>
            <option value="">Selecione...</option>
        ...
        <div>
          ...
          <input type="date" className="w-full p-3 border rounded bg-gray-50 focus:ring-2" value={form.data_pagamento} onChange={(e) => setForm({ ...form, data_pagamento: e.target.value })} required />
        </div>
        <div className="relative">
          <label className="block font-semibold text-gray-700 mb-1">Valor (R$) *</label>
          <style>{`input[type=number]::-webkit-inner-spin-button, input[type=number]::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; }`}</style>
          <input type="number" step="0.01" className="w-full p-3 border rounded bg-gray-50 text-red-600 font-bold" placeholder="0.00" value={form.valor} onChange={(e) => setForm({ ...form, valor: e.target.value })} required />
        </div>
        <div className="md:col-span-2">
          <label className="block font-semibold text-gray-700 mb-1">Observação</label>
          <textarea rows="3" className="w-full p-3 border rounded bg-gray-50" value={form.observacao} onChange={(e) => setForm({ ...form, observacao: e.target.value })}></textarea>
        </div>
        <div className="md:col-span-2 flex justify-end gap-4 border-t pt-6">
          <button type="submit" disabled={loading} className="px-8 py-3 bg-green-600 text-white rounded font-bold shadow-lg"><Save size={20} className="inline mr-2"/> Salvar</button>
        </div>
      </form>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-md relative">
            ...
            <div className="space-y-4">
              <input type="text" placeholder="Nome *" className="w-full p-3 border rounded bg-gray-50" value={novoForn.nome} onChange={e=>setNovoForn({...novoForn, nome:e.target.value})}/>
              <input type="text" placeholder="CPF/CNPJ" className="w-full p-3 border rounded bg-gray-50" value={novoForn.cpf_cnpj} onChange={e=>setNovoForn({...novoForn, cpf_cnpj:e.target.value})}/>
            </div>
            <button onClick={handleSalvarFornecedor} disabled={loading} className="mt-6 w-full py-2 bg-indigo-600 text-white rounded font-bold">Salvar</button>
          </div>
        </div>
      )}
    </div>
